...
export default function PronajemPage() {
  ...
  const fetchProperties = async () => {
    try {
      const response = await fetch('/api/properties?status=PRONAJEM');
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <h1 className="text-4xl font-bold text-red-700">Nemovitosti k pronájmu</h1>
          <p className="text-gray-600 mt-2">
            Aktuálně máme {properties.length} {properties.length === 1 ? 'nemovitost' : properties.length < 5 ? 'nemovitosti' : 'nemovitostí'} k pronájmu
          </p>
        </div>
      </header>
          ...
          {properties.length === 0 ? (
            <div className="text-center py-20">
              ...
              <h3 className="text-xl font-semibold text-gray-600 mb-2">
                Momentálně nemáme žádné nemovitosti k pronájmu
              </h3>
